...
class Slacktion {
  ...
  handleHttpRequest(req, resp) {
    const replyChannel = httpReplyChannel(req, resp)

    const replyFn = reply => csp.putAsync(replyChannel, reply)
      ...
      case 'message':
        ...
        if (message.user !== undefined) {
          ...
          this.bot._api('users.info', {user: message.user}).then(resp => {
            ...
            const replyChannel = rtmReplyChannel(this.bot, this.params, userName)
            const replyFn = reply => csp.putAsync(replyChannel, reply)

            this._dispatchAction(parseMessage(message.text), userName, replyFn)
              .then(res => replyFn(res), err => replyFn(`Error: ${err.message || err}`))
              .then(() => replyChannel.close())
          })
        }
        break
...
function httpReplyChannel(req, resp) {
  const replyChannel = csp.chan(10)

  csp.go(function* () {
    let reply = yield csp.take(replyChannel)

    // First reply will be the HTTP response
    resp.status(200).send(reply + '') // Slack interprets ints as response codes

    // Consecutive replies will be sent through the response url webhook
    while ((reply = yield csp.take(replyChannel)) !== csp.CLOSED) {
      const syncChannel = csp.chan()
      request.post(req.body.response_url).send({ 'response_type': 'ephemeral', text: reply })
        .catch(err => console.error('Error sending response'))
        .then(() => syncChannel.close())

      yield csp.take(syncChannel)
    }
  })
...
function rtmReplyChannel(bot, params, username) {
  const replyChannel = csp.chan(10)
  ...
  csp.go(function* () {
    ...
    while ((reply = yield csp.take(replyChannel)) !== csp.CLOSED) {
      const syncChannel = csp.chan()
      bot.postMessageToUser(username, reply, params)
        .catch(err => console.error('Error sending RTM reply'))
        .then(result => syncChannel.close())
