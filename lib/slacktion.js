class Slacktion {
    ...
    switch (message.type) {
      case 'message':
        if (message.user !== undefined) {
          console.log('Received message from:', message.user, 'With content:\n', message.text)
          ...
          this.bot._api('users.info', {user: message.user}).then(resp => {
            ...
            this._dispatchAction(parseMessage(message.text), userName)
              ...
              .catch(err => this.bot.postMessageToUser(userName, err.message, this.params))
          })
        }
        break
    }
  ...
  async _dispatchAction(message, userName) {
    ...
    const replyFn = async data => this.bot.postMessageToUser(userName, data, this.params)
